...
def update_task(
    ...
    try:
        ...
        if author_id and not _can_edit_task(task, author_id):
            raise TaskPermissionDenied(f"User {author_id} cannot edit task {task_id}")
        
        if resolver_id and not _can_edit_task(task, resolver_id):
...
def add_resolvers_to_task(task_id: str, author_id: str, _resolvers_ids: List[str]) -> Task:
    try:
        ...
        new_resolvers = [get_user(resolver_id) for resolver_id in _resolvers_ids]
...
def remove_resolver_from_task(task_id: str, author_id: str, resolver_id_to_remove: str) -> bool:
    try:
        ...
        resolver_to_remove = get_user(resolver_id_to_remove)
